<template>
    <main>
        ...
        <section class="container mb-3">
            <div class="row justify-content-between">
                <div class="col-auto">
                    <button class="btn btn-primary rounded-3 px-5" @click="newImage()">
                        Nova publicação
                    </button>
                </div>
        ...
        <section class="container">
            <table class="table">
                <thead>
                    <tr>
                        <th class="bg-dark text-light text-center rounded-start">Ordem</th>
                        <th class="bg-dark text-light text-center">Carrossel</th>
                ...
                <tbody class="border">
                    <tr v-for="d in filteredData" :key="d.i_id">
                        <td class="text-center">{{ d.i_order }}</td>
                        <td>{{ d.i_title }}</td>
                        <td class="d-grid">
                        ...
                        <td class="">
                            <div class="form-check form-switch ms-3">
                                <input class="form-check-input" type="checkbox" role="switch"
                                    :checked="d.i_active == 'y'" @change="inactivateImage(d)">
                            </div>
                        </td>
    ...
    <dialog id="caixa" class="rounded-4 border border-secondary">
        <div class="p-3">
            <h3 class="text-center">Editando Imagem</h3>
            ...
            <form @submit.prevent="imageEdit">
                ...
                <div class="mb-2">
                    ...
                    <input type="text" v-model="title" class="form-control border-secondary" required>
                </div>
                <div class="mb-2">
                    <label>Arquivo</label>
...
const editImage = (dt) => {
    order.value = dt.i_order
    title.value = dt.i_title
    active.value = dt.i_active
    imgUrl.value = dt.i_url
    id.value = dt.i_id
...
const filteredData = computed(() => {
    ...
    return data.value.filter(element =>
        element.i_title.toLocaleLowerCase().includes(query)
    )
})
</script>
